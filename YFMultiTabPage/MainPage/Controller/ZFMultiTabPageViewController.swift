//
//  YFMultiTabPageViewController.swift
import Foundation
import UIKit
@objc protocol YFMultiTabPageDelegate: NSObjectProtocol {
    // 横向collectionView滑动
    ...
    // 子列表将要显示
    @objc optional func multiTabPageViewController(_ viewController: YFMultiTabPageViewController, willDisplay index: Int)
    // 子列表显示
    @objc optional func multiTabPageViewController(_ viewController: YFMultiTabPageViewController, displaying index: Int)
    // 子列已经显示
    @objc optional func multiTabPageViewController(_ viewController: YFMultiTabPageViewController, didEndDisplaying index: Int)
}

class YFMultiTabPageViewController: UIViewController {
    
    // MARK: - Public Preproty
    weak var delegate: YFMultiTabPageDelegate?
    
    // MARK: - Private Preproty
    private lazy var mainScrollView: YFCommonTabScrollView = {
        let mainScrollView = YFCommonTabScrollView()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(YFCommonTabItemCell.self, forCellWithReuseIdentifier: NSStringFromClass(YFCommonTabItemCell.self))
    ...
    private var childVCDic: [Int: YFMultiTabChildPageViewController] = [:]
    // 判断是否点击title来滚动页面
    ...
    public func resetChildViewControllers(tabCount: Int) {
        // 清空原来的父控制器
        childVCDic.forEach { (dic: (key: Int, value: YFMultiTabChildPageViewController)) in
...
extension YFMultiTabPageViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: NSStringFromClass(YFCommonTabItemCell.self), for: indexPath) as? YFCommonTabItemCell {
...
extension YFMultiTabPageViewController: YFMultiTabChildPageDelegate {
...
extension YFMultiTabPageViewController: UIScrollViewDelegate {
